...
async fn main() -> Result<()> {
    ...
    let (node_url, faucet_url) = match args.network.to_lowercase().as_str() {
        "devnet" => (
            ...
            Some("https://faucet.devnet.aptoslabs.com"),
        ),
        "testnet" => (
            "https://fullnode.testnet.aptoslabs.com",
            Some("https://faucet.testnet.aptoslabs.com"),
        ),
        "mainnet" => (
    ...
    // Create REST clients
    let rest_client = Client::new(Url::from_str(node_url)?);
    let coin_client = CoinClient::new(&rest_client);

    // Create sender account from private key
    let sender = create_account_from_private_key(&args.private_key)?;
    ...
    // Parse CSV file
    let recipients = parse_csv_file(&args.csv_file)?;
    ...
    println!("Total amount to be sent: {} APT", total_amount as f64 / 100_000_000.0);

    if balance < total_amount {
        return Err(anyhow::anyhow!("Insufficient balance. Need at least {} APT", 
...
fn parse_csv_file(path: &PathBuf) -> Result<Vec<Recipient>> {
    ...
    let address_idx = headers.iter().position(|h| h.to_lowercase() == "address")
        .context("CSV file must have an 'address' column")?;
    
    let amount_idx = headers.iter().position(|h| h.to_lowercase() == "amount")
    ...
    for (i, record) in reader.records().enumerate() {
        ...
        let amount = if amount_str.contains('.') {
            ...
            let whole = parts[0].parse::<u64>().with_context(|| 
                format!("Invalid amount in row {}: {}", i + 1, amount_str))?;
            ...
            amount_str.parse::<u64>().with_context(|| 
                format!("Invalid amount in row {}: {}", i + 1, amount_str))? * 100_000_000
        };
...
async fn process_transfer(
    ...
    let recipient_address = aptos_sdk::types::account_address::AccountAddress::from_hex_literal(&recipient.address)
    ...
    if sender.sequence_number() < on_chain_sequence {
        println!("  ⚠️ Updating sequence number from {} to {}", sender.sequence_number(), on_chain_sequence);
